...
Ceres is a one-stop development application for developers looking to build on top of zilliqa. 
It is a simple to use graphical interface that has multiple zilliqa development tools. 
Development tools that come with Ceres currently are isolated server, Devex developer block explorer and scilla server. 
...
You can download Ceres at https://github.com/Zilliqa/ceres/releases. 
Go to the latest release and choose the relevant file depending on your operating system.
## Available Services
...
1. Server Container (http://localhost:5555) is the local zilliqa blockchain network and it comes with 10 accounts.
...
isolated server. Network Explorer runs on the port 5557. You can access it from any browser on http://localhost:5557.
...
Scilla server runs scilla binaries locally and exposes API endpoints for the scilla-checker and scilla-runner. 
The scilla-server provides the functionality of scilla-runner and scilla-checker as a JSON-RPC server.
The scilla-server process accepts contract execution requests and executes the contract, providing a JSON output within the
server process itself. 

More details about Scilla Server can be found [here](https://github.com/zilliqa/scilla-server). 
Scilla Server runs on port 4000. You can call it's API on http://localhost:4000.

To start the scilla server, you need to click on “Scilla” from the Ceres’ home screen and then click on “Download and install”.
...
In this section, we will deploy a fungible token smart contract using the Zilliqa IDE which would be connected to your 
...
Step 1 : Visit ide.zilliqa.com and start a new file. In this file, paste the code for the fungible token contract (zrc2 contract)
available at https://github.com/Zilliqa/ZRC/blob/master/reference/FungibleToken.scilla and save the file.
...
Note: Network ID for the isolated server is 222, refer https://dev.zilliqa.com/docs/apis/api-blockchain-get-network-id/ to 
know about the id of your zilliqa network.

Step 3: Select an account that you will eventually use to deploy the contract. In this example, we are using a keystore file but you can use ledger or zilpay, which is zilliqa wallet available as a chrome extension similar to metamask.
