...
Token allowance allows the dApp contract to transfer ZRC-2 token, if the current approved allowance is less than `amount`.  If the allowance is sufficient, this method is a no-op. The approval is done by calling `IncreaseAllowance` transition.
