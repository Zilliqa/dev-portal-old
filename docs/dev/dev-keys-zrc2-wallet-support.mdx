...
- fungible
- zilliqa
description: Integratiing ZRC-2 Fungible Tokens Support into Wallets and dApps
---
...
## Check if Contract is Compliant with ZRC-2 Standard

In order to check if the address entered by the user is a ZRC-2 compliant contract, we need to look at the contract code and check if it has the
 required properties. 
 
### Checking Immutable Variables  
For immutable variables, we'll use `GetSmartContractInit` method of Zilliqa API to get the immutable variables of the contract.
...
```
The response received consists of the immutable variable `type`, `name`, and `value`.

To check whether a contract to be compliant with ZRC-2 standard immutable variables requirement, you can check whether has the contract implements the [mandatory immutable variables](#immutable-variables).
...
### Checking Mutable Variables  
For immutable variables, We'll use `GetSmartContractState` method of Zilliqa API to get the mutable variables of the contract.
...
```
The response received above consists of the mutable variable `total_supply`, `balances`, and `allowances`.

To check whether a contract is compliant with ZRC-2 standard mutable variables requirement, you can check whether the contract implements the [mandatory mutable variables](#mutable-variables).
...
### Checking Transitions, Events and Error Codes
Currently, you need to look at the smart contract code manually and check for transitions, events and error codes in a contract. 
To check whether a contract is compliant with ZRC-2 standard error code as well transitions and events requirement, you can check whether the contract implements the [mandatory error codes](#error-codes) and [mandatory transitions and events](#transitions-and-events).
## Integration with ZRC-2 Fungible Tokens contract

### Fetch Token Balance
In a ZRC-2 fungible tokens contract, the mutable field `balances` with data type as a `Map`, stores the mapping between addresses and their 
