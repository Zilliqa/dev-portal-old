...
As the staked seed nodes are for public consumption, we expect high availability, secure and reliable node. Hence, as such all operators is required to support
 serving of api and raw request over SSL/TLS.
