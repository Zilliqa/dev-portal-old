...
In this guide, we will guide you through on how to set up the seed node via
1. Docker
2. Native build
...
<Tabs
  ...
  values={[
    ...
    { label: 'Mainnet (only available after Zilliqa V6.4.0)', value: 'mainnet', }
  ]
}>
...
:::caution
This approach has only been tested on `Ubuntu 18.04.5 LTS` and involves compiling and building the `C++` codebase from scratch. We strongly recommend you consider launching the node using the Docker steps detailed in the previous section.
:::
...
If you have used a port other than 33133, please notify us immediately so that we can adjust our whitelisted port for you.
:::
- Change the value of `ENABLE_WEBSOCKET` to `true`. This enables your seed node to support WebSockets (refer to the [Zilliqa Websocket Server](https://github.com/Zilliqa/dev-portal/tree/master/docs/api-websocket.md) documentation).
...
We will recommend SSN operators to use whitelisting by public key approach.
...
- Add load balancer in front of their node
